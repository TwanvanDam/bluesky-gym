...
class NoiseModel:
    ...
    def get_noise_power_kernel_shape_meters_and_pixels(self, altitude: float) -> tuple[tuple[float, float], tuple[int, int]]:
        noise_radius = np.sqrt(self.base_noise_power_1m / self.cutoff_noise_power) # [ m ]
        noise_radius_rounded = np.ceil(noise_radius / self.config.noise_resolution_m) * self.config.noise_resolution_m
    ...
    def calculate_mean_step_noise(self, population_map: np.ndarray, altitude: float, sim_dt: float) -> float:
        ...
        mean_step_noise_power = self.step_total_noise(mean_population_density, noise_power_kernel, sim_dt)
        return mean_step_noise_power

    @staticmethod
    def step_total_noise(population_map_extract: np.ndarray | float, noise_power_kernel: np.ndarray, sim_dt: float) -> float:
        assert isinstance(population_map_extract, float) or population_map_extract.shape == noise_power_kernel.shape, "Population map extract and noise kernel must have the same shape"
    ...
    def step_normalized_noise(self, population_map_extract: np.ndarray, altitude: float, mean_step_noise: float, sim_dt: float) -> float:
        noise_power_kernel = self.get_noise_power_kernel(altitude)
        step_total_noise = self.step_total_noise(population_map_extract, noise_power_kernel, sim_dt)
        expected_noise = mean_step_noise
        normalized_noise = step_total_noise / expected_noise if expected_noise > 0 else 0
